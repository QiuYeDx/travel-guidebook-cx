...
export function GuidebookMobileToc({ entries }: { entries: TableOfContentsEntry[] }) {
  const [activeId, setActiveId] = useState(entries[0]?.id);
  useEffect(() => {
    const headings = entries.map((entry) => document.getElementById(entry.id)).filter(Boolean) as HTMLElement[];
    if (!headings.length) return;
    const observer = new IntersectionObserver((observations) => {
      const visible = observations.filter((item) => item.isIntersecting).sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top);
  ...
  return <div className="fixed inset-x-4 bottom-[calc(4.25rem+env(safe-area-inset-bottom))] z-40 md:hidden">
    <Popover>
