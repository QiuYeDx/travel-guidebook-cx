...
export function DayGuide({ itinerary }: { itinerary: DayItinerary }) {
  ...
  return <div className="mx-auto w-full max-w-5xl px-4 pb-16 pt-7 sm:px-6 sm:pt-10">
    <header><div className="flex flex-wrap items-center justify-between gap-3"><h1 className="min-w-0 text-3xl font-semibold leading-tight sm:text-4xl">{day.title}</h1><div className="ml-auto flex flex-wrap items-center justify-end gap-2"><Badge className="bg-emerald-700 text-white hover:bg-emerald-700">{day.id}</Badge><Badge variant="outline">{formatTripDate(day.date)}</Badge><Badge variant="outline">{intensityLabels[day.intensity]}</Badge></div></div><p className="mt-3 max-w-2xl text-base leading-7 text-muted-foreground">{day.primaryGoal}</p></header>
