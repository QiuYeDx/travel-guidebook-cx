...
import { ArrowLeftIcon, ArrowRightIcon, ArrowUpRightIcon, BedDoubleIcon, CarFrontIcon, CheckCircle2Icon, Clock3Icon, MapPinIcon, MapPinnedIcon, RouteIcon, ShieldAlertIcon, SparklesIcon } from "lucide-react";
...
function DayNavigation({ itinerary }: { itinerary: DayItinerary }) {
  return <nav aria-label="前后日切换" className="grid grid-cols-2 gap-3">
    {itinerary.previousDay ? <DayLink day={itinerary.previousDay} direction="previous" /> : <span aria-hidden="true" />}
    {itinerary.nextDay ? <DayLink day={itinerary.nextDay} direction="next" /> : <span aria-hidden="true" />}
  </nav>;
}
...
function DayLink({ day, direction }: { day: TripDay; direction: "previous" | "next" }) {
  ...
  return <Link href={`/days/${day.id}`} className={`group flex min-w-0 w-full items-center gap-2 rounded-lg border px-3 py-2 transition-colors hover:bg-accent focus-visible:bg-accent ${alignment}`}>
    {previous ? <ArrowLeftIcon className="size-4 shrink-0 text-muted-foreground transition-transform group-hover:-translate-x-0.5" aria-hidden="true" /> : <span className="size-4 shrink-0" aria-hidden="true" />}
    <span className="min-w-0 flex-1">
      <span className="block text-[0.6875rem] text-muted-foreground">{previous ? "上一日" : "下一日"} · {day.id}</span>
      <span className="mt-1 block min-w-0 text-sm font-medium">
        <span className="hidden truncate sm:block">{day.title}</span>
        <span className={`flex min-w-0 items-center sm:hidden ${previous ? "" : "justify-end"}`}>
          {hasMiddlePoints ? (
            <>
              <span className="shrink-0 whitespace-nowrap">{firstPoint}</span>
              <span className="shrink-0 px-1" aria-hidden="true">…</span>
    ...
    {!previous ? <ArrowRightIcon className="size-4 shrink-0 transition-transform group-hover:translate-x-0.5" aria-hidden="true" /> : <span className="size-4 shrink-0" aria-hidden="true" />}
  </Link>;
}
...
export function DayGuide({ itinerary }: { itinerary: DayItinerary }) {
  const { day } = itinerary;
  const tabItems = [{ value: "overview", label: "总览", icon: <MapPinnedIcon /> }, { value: "route", label: "路线", icon: <RouteIcon /> }, { value: "notes", label: "注意", icon: <ShieldAlertIcon /> }];
  return <div className="mx-auto w-full max-w-5xl px-4 pt-7 sm:px-6 sm:pt-10">
    <header><div className="flex flex-wrap items-center gap-2"><Badge className="bg-emerald-700 text-white hover:bg-emerald-700">{day.id}</Badge><Badge variant="outline">{formatTripDate(day.date)}</Badge><Badge variant="outline">{intensityLabels[day.intensity]}</Badge></div><h1 className="mt-3 text-3xl font-semibold leading-tight sm:text-4xl">{day.title}</h1><p className="mt-3 max-w-2xl text-base leading-7 text-muted-foreground">{day.primaryGoal}</p></header>
    ...
    <div className="mt-7"><ClipPathTabs items={tabItems} defaultValue="overview">
      <TabsContent value="overview" className="mt-0"><section><div className="flex items-end justify-between gap-3"><div><p className="text-xs text-muted-foreground">沿行驶方向排列</p><h2 className="mt-1 text-xl font-semibold">核心停靠与车览</h2></div><Button asChild variant="outline" size="sm"><Link href={`/scenic?day=${day.id}`}>打开观景页 <ArrowRightIcon aria-hidden="true" /></Link></Button></div><div className="mt-4"><ScenicRouteList items={itinerary.scenicSummary} compact /></div></section></TabsContent>
