...
- 清单旧实现仍保留在仓库以供历史数据兼容，但不再作为产品页面或导航入口；后续可独立删除。
...
- 图片使用本地优化资产并控制尺寸。
- 清单和模式在本地保存。
- `/sw.js` 安装时预缓存首页、行程、正式路书、安全、来源、D0-D9 每日页、D0-D9 每日清单参数 URL、D1-D9 观景参数 URL 和必要构建资产 / 图标。
