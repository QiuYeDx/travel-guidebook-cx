...
最终方案采用“内容先行、结构化数据驱动、单一路线、移动端分层阅读、动态信息显式过期”的路线：

- Markdown 保存完整攻略、依据和讨论语境。
- TypeScript 结构化数据保存前端高频读取的日期、路段、状态、触发条件和复核时间。
- 首页只呈现整段路书的总览和最终路线，不区分行前 / 行中，也不把来源、待确认和风险元数据作为首屏主内容。
- 应用只保留一条最终路线；降级动作作为每日页的次级说明，不再以 A / B / C 线路切换呈现。
...
3. 每天只有一个最终路线主目标；遇到触发条件只执行降级动作，不在界面切换另一条线路。
...
- `content/guidebook/2026-chuanxi-grand-loop.md`：v0.2 正式攻略和最终路线沿途观景基线。
- `content/notes/README.md`：讨论与复核记录约定。
- `data/trips/2026-chuanxi/trip.ts`：D0-D9 最终路线、降级动作和触发条件事实源。
...
- `components/layout/mobile-trip-navigation.tsx`：移动端总览、行程、观景和更多导航。
...
- `app/manifest.ts` / `app/sw.js/route.ts` / `features/offline/*` / `lib/offline/*`：可安装 manifest、版本化核心缓存、更新提示、离线链接策略和全局同步状态。
...
- 离线缓存必须先联网完成一次“已就绪”；Service Worker 只缓存同源路书与构建资产，不缓存外部地图、来源或实时状态。
- 清单功能已从产品和仓库移除；安全规则和每日页的注意事项仍保留。
- 中国标准时间当前日推导、手动切日、模式持久化与来源过期状态已实现。
- 旧 `/checklists` 参数契约已不再进入新的导航和阅读路径。
...
| 路由            | 行前模式                                     | 行中模式                                       |
| --------------- | -------------------------------------------- | ---------------------------------------------- |
| `/`             | 整段路线结论、D0-D9 总览、每天主目标、观景概览 | 同一份总览；每日执行细节进入 `/days/[dayId]`   |
| `/itinerary`    | 10 天时间线、海拔与驾驶强度                  | 同一份时间线，快速切日                          |
| `/days/[dayId]` | 单日路线、观景、注意事项和导航               | 同一份三 Tab 单日路书                           |
| `/scenic`       | 按日查看全部停靠点和车览走廊                   | 默认仅看选择日，突出下一处合法停靠和连续车览路段 |
...
桌面端使用紧凑顶部导航；移动端使用顶部标题加底部 4 项导航：
“总览、行程、观景、更多”。“更多”承载完整攻略、安全、来源、项目说明和主题切换，使用受控浮层，点击外部自动关闭。

完整攻略只在全局导航中出现一处，不在总览、行程和每日页重复放“查看完整攻略”按钮。

## 首页与阅读模式

首页固定为“路线总览”：路线摘要、D0-D9 路线带、每天主目标、沿途观景数量和返蓉节点。无需切换行前 / 行中，不展示待确认、来源依据、任务进度和风险列表。

每日执行信息通过每日页进入；页面内部使用 qiuye-ui 的 `ClipPathTabs` / `ResponsiveTabs` 分成“路线、观景、注意”三个内容面板，首屏只保留日期、目的地、当日主目标和下一段导航。
...
- `pnpm test:data` 验证日期唯一、天数连续、ID、来源、`reviewAt`、数值范围和降级触发完整性；
...
离线能力已由 OFFLINE-01 实现，继续遵守以下契约：

- 核心内容在构建期生成。
- 图片使用本地优化资产并控制尺寸。
- 不保存清单或成员隐私状态；`/sw.js` 安装时预缓存首页、行程、正式路书、安全、来源、D0-D9 每日页、D1-D9 观景参数 URL 和必要构建资产 / 图标。
- 文档导航使用 4 秒超时的网络优先策略，构建静态资源使用缓存优先；新版本 worker 保持 waiting，用户点击“刷新更新”后才接管并清理旧版本缓存。
- 全局状态条显示“内容版本 / 最后同步 / 更新可用”；首次缓存未完成或浏览器不支持时不能声称离线已就绪。
- 离线点击同源 Next 链接时强制完整文档导航以命中 HTML 缓存；外部 HTTP(S) 链接被拦截并提示使用复制信息，`tel:` 紧急电话继续放行。
