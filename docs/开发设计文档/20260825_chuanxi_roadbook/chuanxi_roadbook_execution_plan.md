...
下一工作包应为 `OFFLINE-01`，缓存核心阅读路径、D0-D9 每日页和清单参数 URL，并实现内容版本与更新提示。
必须用飞行模式验证真实离线首访 / 复访和版本更新，不能只添加 manifest 或安装外壳。
