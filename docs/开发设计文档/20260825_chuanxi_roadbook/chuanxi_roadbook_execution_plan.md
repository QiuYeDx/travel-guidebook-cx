...
| FE-02 行程时间线与每日页       | 已完成 | 2026-08-25 | `app/itinerary/*`, `app/days/*`, `app/scenic/page.tsx`, `features/itinerary/*`, `lib/navigation/map-links.ts`                         | `test` 28/28、tsc、ESLint、Prettier、production build、390 / 768 / 1440 px 及深色模式浏览器验收通过 | `chuanxi_roadbook_implementation_records/2026-08-25_FE-02_itinerary-and-day-pages.md`      | NAV-01 已补齐设备 URI 与复制退化                                     |
| VIEW-01 观景路线可视化         | 已完成 | 2026-08-25 | `app/scenic/page.tsx`, `features/scenic/*`, `features/itinerary/scenic-day-index.tsx`                                                 | `test` 33/33、tsc、ESLint、Prettier、production build、390 / 768 / 1440 px 及深色模式浏览器验收通过 | `chuanxi_roadbook_implementation_records/2026-08-25_VIEW-01_scenic-route-visualization.md` | server-off 离线已通过；真机飞行模式留给 QA-01                        |
| FE-03 清单与安全页             | 已完成 | 2026-08-25 | `app/checklists/*`, `app/safety/*`, `data/trips/2026-chuanxi/checklists.ts`, `safety.ts`, `features/checklist/*`, `features/safety/*` | `test` 38/38、tsc、ESLint、Prettier、production build、390 / 768 / 1440 px 及深色模式浏览器验收通过 | `chuanxi_roadbook_implementation_records/2026-08-25_FE-03_checklists-and-safety.md`        | localStorage 代码退化已实现；隐私禁用场景未能在当前环境完整模拟      |
| FE-04 行前 / 行中模式          | 已完成 | 2026-08-25 | `features/trip/mode*`, `features/trip/on-trip-dashboard.tsx`, `components/layout/mobile-trip-navigation.tsx`, 全局布局与当日清单深链  | `test` 43/43、tsc、lint、build、390 / 768 / 1440 px、持久化与深链 QA                                | `chuanxi_roadbook_implementation_records/2026-08-25_FE-04_planning-and-on-trip-mode.md`    | OFFLINE-01 已覆盖 D0-D9 清单参数 URL                                 |
...
| CLEANUP-01 移除清单功能           | 已完成 | 2026-08-25 | `app/checklists/*`, `features/checklist/*`, `data/trips/2026-chuanxi/checklists.ts`, `lib/offline/cache-config.ts` | `test` 52/52、tsc、ESLint、production build；无清单路由生成 | `feat/2026-08-25_chuanxi_roadbook_remove-checklists.md` | 历史实施记录中的清单描述仅作记录 |
...
下一工作包应为 `QA-01`，完成全仓回归、可访问性和至少 2 名同行者真机验收。
自动与 server-off 离线验收已完成；QA-01 仍需在手机飞行模式下复核安装、核心页面、更新和操作可达性。
